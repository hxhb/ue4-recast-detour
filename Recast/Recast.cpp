...
// Modified version of Detour's source file
...
static int GetVarArgs(char* Dest, size_t DestSize, const char*& Fmt, char* ArgPtr)
{
...
void rcContext::log(const rcLogCategory category, const char* format, ...)
{
	...
	char* ap;
	va_start(ap, format);
	int len = GetVarArgs(msg, MSG_SIZE, format, ap);
	if (len >= MSG_SIZE)
	{
		len = MSG_SIZE-1;
		msg[MSG_SIZE-1] = '\0';
