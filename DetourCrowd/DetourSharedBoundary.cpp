...
void dtSharedBoundary::Initialize()
{
	...
	for (/*int32*/int Idx = 0; Idx < DT_MAX_AREAS; Idx++)
	{
...
/*int32*/int dtSharedBoundary::CacheData(float* Center, float Radius, dtPolyRef CenterPoly, dtNavMeshQuery* NavQuery, dtQueryFilter* NavFilter)
{
	...
	/*int32*/int DataIdx = FindData(Center, Radius, CenterPoly, NavFilter);
...
/*int32*/int dtSharedBoundary::CacheData(float* Center, float Radius, dtPolyRef CenterPoly, dtNavMeshQuery* NavQuery, /*uint8*/unsigned char SingleAreaId)
{
	...
	/*int32*/int DataIdx = FindData(Center, Radius, CenterPoly, SingleAreaId);
...
{
	const /*int32*/int MaxWalls = 64;
	/*int32*/int NumWalls = 0;
	float WallSegments[MaxWalls * 3 * 2] = { 0 };
	dtPolyRef WallPolys[MaxWalls * 2] = { 0 };

	const /*int32*/int MaxNeis = 64;
	/*int32*/int NumNeis = 0;
	...
	for (/*int32*/int Idx = 0; Idx < NumWalls; Idx++)
	{
	...
	for (/*int32*/int Idx = 0; Idx < NumNeis; Idx++)
	{
		SharedData.Polys.emplace(NeiPolys[Idx]);
	}
}

/*int32*/int dtSharedBoundary::FindData(float* Center, float Radius, dtPolyRef ReqPoly, dtQueryFilter* NavFilter) const
{
	const float RadiusThr = 50.0f;
	const float DistThrSq = FMath::Square(Radius * 0.5f);

	for (/*int32*/int Idx = 0; Idx < Data.Num(); Idx++)
	{
...
/*int32*/int dtSharedBoundary::FindData(float* Center, float Radius, dtPolyRef ReqPoly, /*uint8*/unsigned char SingleAreaId) const
{
	const float DistThrSq = FMath::Square(Radius * 0.5f);
	const float RadiusThr = 50.0f;
...
bool dtSharedBoundary::HasSample(/*int32*/int Idx) const
{
	return (Idx >= 0) && (Idx < Data.GetMaxIndex()) && Data.IsAllocated(Idx);
}

bool dtSharedBoundary::IsValid(/*int32*/int Idx, dtNavMeshQuery* NavQuery, dtQueryFilter* NavFilter) const
{
