	...
	dtNavMeshQuery* navquery, const dtQueryFilter* filter)
{
	...
	m_npolys = FMath::Min(Data.Polys.size(), MAX_LOCAL_POLYS);
	/*int32*/int PolyIdx = 0;
	...
	for (/*int32*/int Idx = 0; Idx < npath; Idx++)
	{
		PathLookup.emplace(path[Idx]);
	}

	m_nsegs = 0;
	for (/*int32*/int Idx = 0; Idx < Data.Edges.size(); Idx++)
	{
		float tseg = 0.0f;
		...
		bool bConrainP0 = PathLookup.find(Data.Edges[Idx].p0) != PathLookup.end();
		bool bConrainP1 = PathLookup.find(Data.Edges[Idx].p1) != PathLookup.end();
		if (bConrainP0 && bConrainP1)
		{
			continue;
		}
