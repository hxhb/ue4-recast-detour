...
public:
	...
	FSparseArrayAllocationInfo AllocateIndex(int Index)
	{
		check(Index >= 0);
	...
	FSparseArrayAllocationInfo AddUninitializedAtLowestFreeIndex(int& LowestFreeIndexSearchStart)
	{
		int Index;
		if(NumFreeIndices)
		{
	 ...
	 */
	int AddAtLowestFreeIndex(const ElementType& Element, int& LowestFreeIndexSearchStart)
	{
	 ...
	 * to construct the element in the allocated memory.
	 */
	FSparseArrayAllocationInfo InsertUninitialized(int Index)
	{
		...
		if(Index >= Data.Num())
		{
			...
			{
				const int FreeIndex = AllocationFlags.Num();
		...
		--NumFreeIndices;
		const int PrevFreeIndex = GetData(Index).PrevFreeIndex;
		const int NextFreeIndex = GetData(Index).NextFreeIndex;
		if(PrevFreeIndex != -1)
		{
	 ...
	 */
	void Insert(int Index,typename TTypeTraits<ElementType>::ConstInitType Element)
	{
	...
	void RemoveAt(int Index,int Count = 1)
	{
		...
		{
			for (int It = Index, ItCount = Count; ItCount; ++It, --ItCount)
			{
	...
	void RemoveAtUninitialized(int Index,int Count = 1)
	{
		for (; Count; --Count)
		{
			check(AllocationFlags[Index]);
	...
	void Reserve(int ExpectedNumElements)
	{
		if ( ExpectedNumElements > Data.Num() )
		{
			...
			int ElementIndex = Data.AddUninitialized(ElementsToAdd);

			// now mark the new elements as free
			for ( int FreeIndex = ExpectedNumElements - 1; FreeIndex >= ElementIndex; --FreeIndex )
			{
				if(NumFreeIndices)
				{
					GetData(FirstFreeIndex).PrevFreeIndex = FreeIndex;
	...
	void Shrink()
	{
		...
		int MaxAllocatedIndex = AllocationFlags.FindLast(true);
		...
		if(FirstIndexToRemove < Data.Num())
		{
			if(NumFreeIndices > 0)
			{
				...
				while(FreeIndex != INDEX_NONE)
				{
					...
					{
						const int PrevFreeIndex = GetData(FreeIndex).PrevFreeIndex;
						const int NextFreeIndex = GetData(FreeIndex).NextFreeIndex;
						if(NextFreeIndex != -1)
						{
							GetData(NextFreeIndex).PrevFreeIndex = PrevFreeIndex;
						}
	...
	bool Compact()
	{
		...
		int EndIndex    = Data.Num();
		int TargetIndex = EndIndex - NumFree;
		int FreeIndex   = FirstFreeIndex;
		while (FreeIndex != -1)
		{
			int NextFreeIndex = GetData(FreeIndex).NextFreeIndex;
	...
	TSparseArray& operator=(const TSparseArray& InCopy)
	{
		if (this != &InCopy)
		{
			...
			if (!TIsTriviallyCopyConstructible<ElementType>::Value)
			{
				...
				for (int Index = 0; Index < SrcMax; ++Index)
				{
...
public:
	...
	int PointerToIndex(const ElementType* Ptr) const
	{
		checkSlow(Data.Num());
		int Index = Ptr - &GetData(0);
	...
	bool IsAllocated(int Index) const { return AllocationFlags[Index]; }
...
{
	check( Size == sizeof( T ) );
	const int Index = Array.AddUninitialized().Index;
	return &Array[ Index ];
}
template <typename T, typename Allocator> void* operator new( size_t Size, TSparseArray<T, Allocator>& Array, int Index )
{
	check( Size == sizeof( T ) );
...
{
public:
	static FScriptSparseArrayLayout GetScriptLayout(int ElementSize, int ElementAlignment)
	{
		FScriptSparseArrayLayout Result;
		Result.Alignment     = FMath::Max(ElementAlignment, (int)alignof(FFreeListLink));
		Result.Size          = FMath::Max(ElementSize,      (int)sizeof (FFreeListLink));
	...
	void* GetData(int Index, const FScriptSparseArrayLayout& Layout)
	{
		return (uint8*)Data.GetData() + Layout.Size * Index;
	}

	const void* GetData(int Index, const FScriptSparseArrayLayout& Layout) const
	{
	...
	void Empty(int Slack, const FScriptSparseArrayLayout& Layout)
	{
		// Free the allocated elements.
	 ...
	 */
	int AddUninitialized(const FScriptSparseArrayLayout& Layout)
	{
		int Index;
		if (NumFreeIndices)
		{
	...
	/** Removes Count elements from the array, starting from Index, without destructing them. */
	void RemoveAtUninitialized(const FScriptSparseArrayLayout& Layout, int Index, int Count = 1)
	{
		for (; Count; --Count)
		{
			check(AllocationFlags[Index]);
...
private:
	...
	static void CheckConstraints()
	{
		...
		typedef TSparseArray<int> RealType;
	...
	/** Accessor for the element or free list data. */
	FORCEINLINE FFreeListLink* GetFreeListLink(int Index, const FScriptSparseArrayLayout& Layout)
	{
